...
use super::{
	BeatmapFile, BeatmapFileParseError, Color, ColorParseError, ColorsSection, CurvePointsParseError,
	DifficultySection, EditorSection, Event, EventParams, EventParseError, GeneralSection, HitObject, HitObjectParams,
	HitObjectParseError, HitObjectType, HitSample, HitSampleParseError, HitSampleSet, HitSound,
	InvalidKeyValuePairError, InvalidListError, MetadataSection, SectionParseError, SliderCurveType, SliderPoint,
	TimingPoint, TimingPointParseError, UnspecifiedFieldError,
};
...
pub(crate) fn parse_field_value_pair(line: &str) -> Result<(String, String), InvalidKeyValuePairError> {
	let (field, value) = line.split_once(':').ok_or_else(|| {
		Report::new(InvalidKeyValuePairError::from(line)).attach_printable("Could not split with ':'")
	})?;
...
pub(crate) fn parse_list_of_with_sep<T, E>(line: &str, sep: char) -> Result<Vec<T>, InvalidListError<T>>
where
	...
	loop {
		if let Some(line) = reader.next() {
			...
			match field.as_str() {
				...
				"StackLeniency" => {
					section.stack_leniency = section_fvp_rctx!(value.parse(), General, StackLeniency)?;
				}
				...
				"UseSkinSprites" => {
					section.use_skin_sprites = section_fvp_rctx!(value.parse::<u8>(), General, UseSkinSprites)? != 0;
				}
				"AlwaysShowPlayfield" => {
				...
				"OverlayPosition" => {
					section.overlay_position = section_fvp_rctx!(value.parse(), General, OverlayPosition)?;
				}
				"SkinPreference" => section.skin_preference = Some(value),
				"EpilepsyWarning" => {
					section.epilepsy_warning = section_fvp_rctx!(value.parse::<u8>(), General, EpilepsyWarning)? != 0;
				}
				"CountdownOffset" => {
					section.countdown_offset = section_fvp_rctx!(value.parse(), General, CountdownOffset)?;
				}
				"SpecialStyle" => {
					section.special_style = section_fvp_rctx!(value.parse::<u8>(), General, SpecialStyle)? != 0;
				}
				...
				"SamplesMatchPlaybackRate" => {
					section.samples_match_playback_rate =
						section_fvp_rctx!(value.parse::<u8>(), General, SamplesMatchPlaybackRate)? != 0;
				}
...
fn parse_editor_section(
	...
	loop {
		if let Some(line) = reader.next() {
			...
			match field.as_str() {
				...
				"DistanceSpacing" => {
					distance_spacing = Some(section_fvp_rctx!(value.parse(), Editor, DistanceSpacing)?);
				}
				"BeatDivisor" => {
	...
	Ok(EditorSection {
		...
		beat_divisor: beat_divisor.ok_or_else(|| {
			Report::new(UnspecifiedFieldError::from("BeatDivisor")).change_context(SectionParseError::from("Editor"))
		})?,
		grid_size: grid_size.ok_or_else(|| {
			Report::new(UnspecifiedFieldError::from("GridSize")).change_context(SectionParseError::from("Editor"))
		})?,
		timeline_zoom,
	})
}
	...
	loop {
		if let Some(line) = reader.next() {
			...
			match field.as_str() {
				...
				"BeatmapID" => {
					section.beatmap_id = Some(section_fvp_rctx!(value.parse(), Metadata, BeatmapID)?);
				}
				"BeatmapSetID" => {
					section.beatmap_set_id = Some(section_fvp_rctx!(value.parse(), Metadata, BeatmapSetID)?);
				}
...
fn parse_difficulty_section(
	...
	loop {
		if let Some(line) = reader.next() {
			...
			match field.as_str() {
				"HPDrainRate" => {
					section.hp_drain_rate = section_fvp_rctx!(value.parse(), Difficulty, HPDrainRate)?;
				}
				"CircleSize" => {
					section.circle_size = section_fvp_rctx!(value.parse(), Difficulty, CircleSize)?;
				}
				"OverallDifficulty" => {
					section.overall_difficulty = section_fvp_rctx!(value.parse(), Difficulty, OverallDifficulty)?;
				}
				"ApproachRate" => {
					section.approach_rate = section_fvp_rctx!(value.parse(), Difficulty, ApproachRate)?;
				}
				"SliderMultiplier" => {
					section.slider_multiplier = section_fvp_rctx!(value.parse(), Difficulty, SliderMultiplier)?;
				}
				"SliderTickRate" => {
					section.slider_tick_rate = section_fvp_rctx!(value.parse(), Difficulty, SliderTickRate)?;
				}
...
fn parse_event(line: &str) -> Result<Option<Event>, EventParseError> {
	...
	let event_type: String = values
		.next()
		.ok_or_else(|| Report::new(EventParseError::from(line)).attach_printable("Event is empty".to_owned()))?
		.trim()
		.to_owned();
	...
	let start_time: f64 = {
		let s = values.next().ok_or_else(|| {
			Report::new(EventParseError::from(line)).attach_printable("Event does not have a start time".to_owned())
		})?;
	...
	let params: EventParams = match event_type.as_str() {
		...
		t => {
			return Err(Report::new(EventParseError::from(line)).attach_printable(format!("Unknown event type: {t:?}")));
		}
	};
...
fn parse_color(line: &str) -> Result<Color, ColorParseError> {
	...
	if let [r, g, b] = nums[..] {
		...
		Err(Report::from(ColorParseError::from(line)).attach_printable("Expected 3 or 4 numbers between 0 ad 255"))
	}
}
...
fn parse_hit_sample(line: &str) -> Result<HitSample, HitSampleParseError> {
	...
	let hit_sample = if let [normal_set, addition_set, leftover @ ..] = &args[..] {
		...
		bail!(Report::from(HitSampleParseError::from(line)).attach_printable(format!(
...
fn parse_curve_points(line: &str) -> Result<(SliderCurveType, Vec<SliderPoint>), CurvePointsParseError> {
	...
	let first_curve_token = curve_tokens.next().ok_or_else(|| CurvePointsParseError::from(line))?;
...
fn parse_hit_object(line: &str) -> Result<HitObject, HitObjectParseError> {
	...
	if let [x, y, time, object_type, hit_sound, object_params @ ..] = &args[..] {
		...
		let object_params = {
			if HitObject::raw_is_hit_circle(object_type) {
				...
				if let [curve_points, slides, length, leftover @ ..] = object_params {
					let (first_curve_type, curve_points) =
						parse_curve_points(curve_points).change_context_lazy(|| HitObjectParseError::from(line))?;
					...
					if let [ehitsounds, esamplesets, hit_sample] = leftover {
						...
						edge_samplesets = parse_list_of_with_sep::<HitSampleSet, _>(esamplesets, '|')
							.change_context_lazy(|| HitObjectParseError::from(line))?;
					...
					bail!(Report::new(HitObjectParseError::from(line)).attach_printable(format!(
		...
		let hit_sample = match hit_sample_leftover {
			...
			Some(hit_sample_leftover) => {
				parse_hit_sample(hit_sample_leftover).change_context_lazy(|| HitObjectParseError::from(line))?
			}
			_ => HitSample::default(),
		};
		...
		Err(Report::from(HitObjectParseError::from(line)).attach_printable(format!(
	...
	P: AsRef<Path>,
{
	...
	let fformat_string = reader.next().ok_or_else(|| {
		Report::new(BeatmapFileParseError::from(filename)).attach_printable("File is empty".to_owned())
	})??;
